...
from sbsde.loss import compute_loss, compute_loss_s_u_2
...
def main(interval, sde_cls, mode):
        ...
        for t in sde.ts:
            with torch.no_grad():
                if mode == Mode.SB:
                    ...
                    u = net1(t, x)
                    x = sde.propagate_s_u_2(t, x, u, direction=Direction.FORWARD)
        ...
        for t in reversed(sde.ts):
            with torch.no_grad():
                if mode == Mode.SB:
                    ...
                    u2 = net3(t, x)
                    s = net2(t, x)
                    x = sde.propagate_s_u_2(
                        t, x, u, u2=u2, s=s, direction=Direction.BACKWARD
                    )
    ...
    for it in bar:
        ...
        xs, us, x_term_f = sde.sample_traj(
            net1, mode="normal" if mode == Mode.SB else "s_u_2"
        )
        ...
        if mode == Mode.SB:
            ...
            loss = compute_loss_s_u_2(
                sde=sde, net_u2=net3, net_s=net2, ts=_ts, xs=xs, x_term=x_term_f, us=us
            )
        loss.backward()
        optimizer1.step()
        optimizer2.step()
